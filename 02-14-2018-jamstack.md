...
Let's actually write some code! Start the development server by running `npm run develop`. Go ahead and delete `src/pages/page-2.js` assuming your version of `gatsby-cli` generated that file, we won't be needing it. Also delete the `<Link>` component in `pages/index.js` as that pages no longer exists.
...
4. `headerColor` being passed to the `HeaderContainer` styled-component. This is what makes styled-components powerful. It allows you to pass in dynamic data to your components's styles giving you flexability with your styling that you otherwise wouldn't have with regular css files.
